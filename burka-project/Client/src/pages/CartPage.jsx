...
const CartPage = () => {
  ...
  const generatePDF = () => {
    ...
    if (vendor) {
      doc.setFontSize(12);
      doc.text(`Vendor: ${vendor.firmName}`, 10, 20);
      doc.text(`Vendor ID: ${vendor._id}`, 10, 25);
    }
  ...
  const handlePayment = () => {
    ...
    setTimeout(() => {
      ...
      Promise.all(
        ...
        .then(() => {
          fetchCart();
          generatePDF(); // Generate PDF after successful payment
        })
        .finally(() => {
          setTimeout(() => {
            closeModal();
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-lg p-6 max-w-md w-full mx-4 animate-fade-in">
            ...
            <div className="flex flex-col gap-3">
              ...
              {!paymentSuccess && (
  <button
    onClick={closeModal} // Corrected: use a single function call
    disabled={processingPayment}
    className="px-4 py-2 rounded-lg border hover:bg-gray-50 transition-colors text-gray-700 disabled:opacity-50"
  >
    Cancel
